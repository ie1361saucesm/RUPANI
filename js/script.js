...
let DB = {
  ...
  cfgUrl: '', cfgConectado: false, cfgError: '', autoSync: false,
...
function saveLocalDB() {
  const dbToSave = { ...DB };
  // Excluir propiedades temporales
  delete dbToSave.session;
  delete dbToSave.estSession;
  delete dbToSave.modal;
  delete dbToSave._clockTimer;
  delete dbToSave.cfgError; // temporal
  localStorage.setItem('rupaniDB', JSON.stringify(dbToSave));
}

function loadLocalDB() {
  const saved = localStorage.getItem('rupaniDB');
  if (saved) {
    try {
      const loaded = JSON.parse(saved);
      // Fusionar con DB por defecto
      DB = { ...DB, ...loaded };
      // Restaurar propiedades temporales
      DB.session = null;
      DB.estSession = null;
      DB.modal = null;
      DB._clockTimer = null;
      ...
      console.warn('Error cargando DB local:', e);
    }
  }
}
...
async function sheetsCargarTodo() {
  DB.cfgError = '';
  if (!DB.cfgUrl) { DB.cfgConectado = false; return false; }
  try {
    // Primero, subir datos locales a Sheets si hay conexión
    if (DB.cfgConectado) {
      await sheetsSyncUp();
    }
...
function renderSelector() {
  return '<div class="lw"><div class="lb">' +
    ...
    '<img src="img/rupani.png" alt="logo" style="width:180px">' +
    '</div>' +
...
function renderAdminLogin() {
  return '<div class="lw"><div class="lb">' +
    ...
    '<img src="img/rupani.png" alt="logo rupani" style="width:180px">' +
...
function bindEstSetup(esForzado) {
  ...
  q('#btn-setup-save').onclick = async () => {
    ...
    if (est) {
      est.usuario = usr; est.password = pass; est.credCreadas = true;
      await sheetsUpdateMaybe('est', est.id, est);
    }
  ...
  if (tipo === 'edit-apod') {
    q('#m-save').onclick = async () => {
      ...
      if (datos && datos.id) {
        ...
        if (a) {
          a.nombres = nom; a.dir = dir; a.cel = cel; a.correo = cor;
          await sheetsUpdateMaybe('apod', a.id, a);
          saveLocalDB();
        }
      } else {
        const nuevo = { id: nid(), nombres: nom, dir: dir, cel: cel, correo: cor };
        DB.apods.push(nuevo);
        await sheetsSaveMaybe('apod', nuevo);
      }
      saveLocalDB();
    ...
    q('#m-reset-cred') && q('#m-reset-cred').addEventListener('click', async () => {
      ...
      if (est) {
        est.usuario = ''; est.password = ''; est.credCreadas = false;
        await sheetsUpdateMaybe('est', est.id, est);
      }
      saveLocalDB();
    ...
    q('#m-save').onclick = async () => {
      ...
      if (datos && datos.id) {
        ...
        if (e) {
          ...
          await sheetsUpdateMaybe('est', e.id, e);
          saveLocalDB();
        }
      } else {
        const id = nid();
        const codigo = 'EST-' + id.toString().slice(-3);
        const nuevo = { id, apodId, nombres: nom, edad, grado, cel, correo: cor, codigo, usuario: '', password: '', credCreadas: false };
        DB.ests.push(nuevo);
        await sheetsSaveMaybe('est', nuevo);
      }
      saveLocalDB();
    ...
    q('#m-save').onclick = async () => {
      ...
      if (datos && datos.id) {
        ...
        if (m) {
          m.estId = estId; m.monto = monto; m.desde = desde; m.hasta = hasta; m.fecha = fecha; m.pagado = pagado;
          await sheetsUpdateMaybe('mat', m.id, m);
          saveLocalDB();
        }
      } else {
        const nuevo = { id: nid(), num, estId, fecha, monto, desde, hasta, pagado };
        DB.mats.push(nuevo);
        await sheetsSaveMaybe('mat', nuevo);
      }
      saveLocalDB();
    ...
    q('#m-save').onclick = async () => {
      ...
      if (datos && datos.id) {
        ...
        if (a) {
          a.nombres = nom; a.email = email; if (pass) a.password = pass;
          await sheetsUpdateMaybe('admin', a.id, a);
          saveLocalDB();
        }
      } else {
        if (!usr) { toast('Ingresa un usuario.', 'wa'); return; }
        if (DB.admins.find(a => a.usuario === usr)) { toast('El usuario ya existe.', 'wa'); return; }
        const nuevo = { id: nid(), nombres: nom, usuario: usr, password: pass || 'rupani123', email, esSuperAdmin: false };
        DB.admins.push(nuevo);
        await sheetsSaveMaybe('admin', nuevo);
      }
      saveLocalDB();
    ...
    q('#m-save').onclick = async () => {
      ...
      if (datos && datos.id) {
        ...
        if (s) {
          s.titulo = titulo; s.fecha = fecha; s.total = total;
          await sheetsUpdateMaybe('sim', s.id, s);
          saveLocalDB();
        }
      } else {
  ...
  document.querySelectorAll('.sim-crud-del').forEach(b => {
    b.addEventListener('click', () => {
      ...
      confirmDelete('¿Eliminar "' + (s ? s.titulo : 'este simulacro') + '"? Se perderán todos sus resultados.', async () => {
        ...
        await sheetsDeleteMaybe('sim', +b.dataset.sid);
        saveLocalDB();
  ...
  document.querySelectorAll('.sim-res-del').forEach(b => {
    b.addEventListener('click', () => {
      ...
      confirmDelete('¿Eliminar el resultado de ' + e.nombres + '?', async () => {
        ...
        await sheetsUpdateMaybe('sim', sim.id, [sim.id, sim.titulo, sim.fecha, sim.total, JSON.stringify(sim.res)]);
        saveLocalDB();
...
`;
  ...
  html += '<div style="flex:1"><div style="font-weight:700;font-size:13.5px;margin-bottom:4px">Crear Apps Script en tu hoja de Google Sheets</div>';
  html += '<div style="font-size:12.5px;color:var(--t2);line-height:1.7">Abre tu hoja en <strong>sheets.google.com</strong> → <strong>Extensiones → Apps Script</strong><br>Borra TODO el código y pega el que aparece abajo.</div>';
  html += '</div></div>';
  ...
  html += '<div style="flex:1"><div style="font-weight:700;font-size:13.5px;margin-bottom:4px">Pegar el Apps Script</div>';
  html += '<div style="font-size:12.5px;color:var(--t2);margin-bottom:10px">En tu hoja ve a <strong>Extensiones → Apps Script</strong>, borra todo y pega este código.</div>';
  html += '<div style="position:relative"><pre style="background:#1e1e2e;border-radius:10px;padding:16px;font-family:monospace;font-size:11px;color:#cdd6f4;line-height:1.75;overflow-x:auto;white-space:pre-wrap;margin-top:12px" id="script-code">' + scriptCode + '</pre>';
...
function bindView() {
    ...
    document.querySelectorAll('.crud-del').forEach(b => b.addEventListener('click', () => {
      ...
      confirmDelete(msgs[tipo], async () => {
        if (tipo === 'apod') {
          ...
          for (const h of hijos) { for (const m of DB.mats.filter(mm => mm.estId === h.id)) await sheetsDeleteMaybe('mat', m.id); DB.mats = DB.mats.filter(m => m.estId !== h.id); await sheetsDeleteMaybe('est', h.id); }
          DB.ests = DB.ests.filter(e => e.apodId !== id); DB.apods = DB.apods.filter(x => x.id !== id); await sheetsDeleteMaybe('apod', id);
          saveLocalDB();
        } else if (tipo === 'est') {
          for (const m of DB.mats.filter(mm => mm.estId === id)) await sheetsDeleteMaybe('mat', m.id);
          DB.mats = DB.mats.filter(m => m.estId !== id); DB.ests = DB.ests.filter(x => x.id !== id); await sheetsDeleteMaybe('est', id);
          saveLocalDB();
        } else if (tipo === 'mat') {
          DB.mats = DB.mats.filter(x => x.id !== id); await sheetsDeleteMaybe('mat', id);
          saveLocalDB();
        } else if (tipo === 'admin') {
          DB.admins = DB.admins.filter(x => x.id !== id); await sheetsDeleteMaybe('admin', id);
          saveLocalDB();
        }
  ...
  if (DB.view === 'matricular') {
    ...
    q('#mat-save') && q('#mat-save').addEventListener('click', async () => {
      ...
      if (DB.matApodMode === 'existente') { aid = +DB.matApodSel; }
        ...
        aid = nid();
        const nuevoApod = { id: aid, ...DB.matAF };
        DB.apods.push(nuevoApod);
        await sheetsSaveMaybe('apod', nuevoApod);
        saveLocalDB();
      }
      const eid = nid() + 1, newId = nid() + 2;
      const codigo = 'EST-' + String(eid).slice(-3);
      const nuevoEst = { id: eid, apodId: aid, ...DB.matEF, edad: +DB.matEF.edad, codigo, usuario: '', password: '', credCreadas: false };
      DB.ests.push(nuevoEst);
      await sheetsSaveMaybe('est', nuevoEst);
      saveLocalDB();
      const num = 'MAT-' + anioActual() + '-' + String(DB.mats.length + 1).padStart(3, '0');
      const nuevaMat = { id: newId, num, estId: eid, fecha: DB.matMF.fecha || hoy(), monto: +DB.matMF.monto, desde: DB.matMF.desde || hoyMes(), hasta: DB.matMF.hasta || hoyMes(), pagado: false };
      DB.mats.push(nuevaMat);
      await sheetsSaveMaybe('mat', nuevaMat);
      saveLocalDB();
  ...
  if (DB.view === 'pagos') {
    ...
    document.querySelectorAll('.pago-btn').forEach(b => b.addEventListener('click', () => { const m = DB.mats.find(x => x.id === +b.dataset.id); if (m) { m.pagado = true; saveLocalDB(); toast('Pago confirmado.'); render(); } }));
  }
  if (DB.view === 'buscar') {
  ...
  if (DB.view === 'config') {
    ...
    q('#cfg-auto-sync-btn') && q('#cfg-auto-sync-btn').addEventListener('click', () => {
      DB.autoSync = !DB.autoSync;
      saveLocalDB();
    ...
    q('#cfg-send-btn') && q('#cfg-send-btn').addEventListener('click', async () => {
      if (!DB.cfgUrl) { toast('No hay URL guardada.', 'wa'); return; }
      const btn = q('#cfg-send-btn'); if (btn) { btn.textContent = '⏳ Enviando...'; btn.disabled = true; }
      toast('Enviando datos a Sheets...');
      await sheetsSyncUp();
      toast('✓ Envío completado.', 'ok');
      render();
    });
    q('#cfg-load-btn') && q('#cfg-load-btn').addEventListener('click', async () => {
      if (!DB.cfgUrl) { toast('No hay URL guardada.', 'wa'); return; }
      const btn = q('#cfg-load-btn'); if (btn) { btn.textContent = '⏳ Cargando...'; btn.disabled = true; }
      toast('Cargando datos desde Sheets...');
      const ok = await sheetsCargarTodo();
      toast(ok ? '✓ Datos cargados desde Sheets.' : 'No se pudieron cargar los datos.', ok ? 'ok' : 'wa');
      if (ok) saveLocalDB();
      render();
    });
    q('#cfg-reload-btn') && q('#cfg-reload-btn').addEventListener('click', async () => {
      ...
      toast('Recargando datos...');
      const ok = await sheetsCargarTodo();
      toast(ok ? '✓ Datos actualizados.' : 'No se pudieron cargar los datos.', ok ? 'ok' : 'wa');
      render();
    });
    q('#cfg-reset-btn') && q('#cfg-reset-btn').addEventListener('click', () => {
      if (!confirm('¿Desconectar de Google Sheets?')) return;
      DB.cfgConectado = false;
      saveLocalDB();
      render();
    });
...
async function initApp() {
  ...
  render();
  if (DB.cfgUrl) {
    toast('Restaurando datos guardados...');
    const ok = await sheetsCargarTodo();
    toast(ok ? '✓ Datos cargados desde Google Sheets.' : 'Usando datos locales. No se pudo conectar con Sheets.', ok ? 'ok' : 'wa');
    render();
  }
}
initApp();
